...
if SSH_ENABLED:
    ...
    # LLM_GATEWAY_HOST = "127.0.0.1" # Todo
else:
    ...
    # LLM_GATEWAY_HOST = os.getenv("LLM_GATEWAY_HOST", REMOTE_HOST) # Todo
...
BE_ROUTER_URL = f"http://{REMOTE_HOST}:{BE_ROUTER_PORT}"

GM_PORT = os.getenv("GM_PORT")
GM_SERVICE_URL = f"http://{REMOTE_HOST}:{GM_PORT}"

SCENARIO_SERVICE_PORT = os.getenv("SCENARIO_SERVICE_PORT")
SCENARIO_SERVICE_URL = f"http://{REMOTE_HOST}:{SCENARIO_SERVICE_PORT}"
...
LLM_GATEWAY_URL = f"http://{REMOTE_HOST}:{LLM_GATEWAY_PORT}"

WEB_PORT = os.getenv("WEB_PORT")
WEB_URL = f"http://{REMOTE_HOST}:{WEB_PORT}"